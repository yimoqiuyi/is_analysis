        ...
        <div class="layui-side-scroll">
            ...
            <ul class="layui-nav layui-nav-tree" lay-filter="test">
                <li class="layui-nav-item layui-nav-itemed">
                    <a class=""
                       href="${pageContext.request.contextPath}/admin/permissionAssignment.do">
                        权限-角色分配</a>
                </li>
                <li class="layui-nav-item layui-nav-itemed">
                    <a class=""
                       href="${pageContext.request.contextPath}/admin/roleAssignment.do">
                        角色-用户分配</a>
                </li>
             </ul>
        </div>
        ...
        <div style="padding: 15px;">
            ...
            <script type="text/html" id="barDemo">
                <a class="layui-btn layui-btn-xs" lay-event="edit">分配权限</a>
                 <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="detail">角色权限</a>
            </script>
    ...
    layui.use(['laypage', 'layer', 'table', 'element', 'slider'], function () {
        ...
        table.render({
            ...
            , cols: [[ //表头
                ...
                , {field: 'roleId', title: 'ID', width: 80, sort: true, fixed: 'left', totalRowText: '合计：'}
                , {field: 'name', title: '角色名字', width: 120}
                , {field: 'introduce', title: '角色注解', width: 150}
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                case 'add':
                    ...
                    window.location.href = "${pageContext.request.contextPath}/admin/insertOneRole.do?roleId=" + id;
                    layer.msg('添加');
                    break;
                case 'update':
                    if (data.length === 0) {
                        ...
                        var id = checkStatus.data[0].roleId;
                        window.location.href = "${pageContext.request.contextPath}/admin/updateOneRole.do?roleId=" + id;
                ...
                case 'delete':
                    if (data.length === 0) {
                        ...
                        var id = checkStatus.data[0].roleId;
                        window.location.href = "${pageContext.request.contextPath}/admin/deleteOneRole.do?roleId=" + id;
                        layer.msg('删除');
                    }
                    break;
            }
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                layer.confirm('查看', function (index) {
                    window.location.href = "${pageContext.request.contextPath}/admin/checkRolePermission.do?roleId=" + data.roleId
                    obj.del(); //删除对应行（tr）的DOM结构
                    layer.close(index);
                    //向服务端发送删除指令
                });
            } else if (layEvent === 'edit') {
                window.location.href = "${pageContext.request.contextPath}/admin/rolePermissionDistribution.do?roleId=" + data.roleId
                layer.msg('编辑操作');
            }
        });
        //分页
