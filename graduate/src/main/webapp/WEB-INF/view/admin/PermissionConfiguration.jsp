    ...
    layui.use(['laypage', 'layer', 'table', 'element', 'slider'], function () {
        ...
        table.render({
            ...
            , cols: [[ //表头
                ...
                , {field: 'name', title: '权限名字', width: 120}
                , {field: 'node', title: '权限注解', width: 180}
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                case 'add':
                    ...
                    window.location.href = "${pageContext.request.contextPath}/admin/insertOnePermission.do?id=" + id;
                    layer.msg('添加');
                    break;
                case 'update':
                    if (data.length === 0) {
                        ...
                        window.location.href = "${pageContext.request.contextPath}/admin/selectOnePermission.do?id=" + id;
                ...
                case 'delete':
                    if (data.length === 0) {
                        ...
                        var id = checkStatus.data[0].userId;
                        window.location.href = "${pageContext.request.contextPath}/admin/deleteOnePermission.do?id=" + id;
                        layer.msg('删除');
                    }
                    break;
            }
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                window.location.href = "${pageContext.request.contextPath}/admin/selectOnePermission.do?id=" + data.id
                layer.msg('查看操作');
            } else if (layEvent === 'del') {
                layer.confirm('真的删除行么', function (index) {
                    window.location.href = "${pageContext.request.contextPath}/admin/deleteOnePermission.do?id=" + data.id
                    obj.del(); //删除对应行（tr）的DOM结构
                    layer.close(index);
                    //向服务端发送删除指令
                });
            } else if (layEvent === 'edit') {
                window.location.href = "${pageContext.request.contextPath}/admin/insertOnePermission.do"
                layer.msg('添加');
            }
        });
        //分页
