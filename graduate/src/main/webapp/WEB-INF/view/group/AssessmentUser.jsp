...
<div class="layui-layout layui-layout-admin">
    ...
    <script type="text/html" id="barDemo">
        <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail">卫生情况</a>
        <a class="layui-btn layui-btn-xs" lay-event="detai2">考勤情况</a>
    </script>
    ...
    layui.use(['laypage', 'layer', 'table', 'element', 'slider'], function () {
        ...
        table.render({
            ...
            , cols: [[ //表头
                ...
                , {field: 'userId', title: 'ID', width: 60, sort: true, fixed: 'left', totalRowText: '合计：'}
        ...
        //监听头工具栏事件obj.config.id
        table.on('toolbar(test)', function (obj) {
            var checkStatus = table.checkStatus('table1')
                , data = checkStatus.data; //获取选中的数据
            // alert(checkStatus.data[0].userId) //获取用户id
            switch (obj.event) {
                case 'add':
                ...
                case 'update':
                    if (data.length === 0) {
                        ...
                        window.location.href = "${pageContext.request.contextPath}/${pageContext.request.contextPath}/user/select.do?userId=" + id;
                ...
                case 'delete':
                    if (data.length === 0) {
                        ...
                        var id = checkStatus.data[0].userId;
                        window.location.href = "${pageContext.request.contextPath}/user/delete.do?userId=" + id;
                        layer.msg('删除');
                    }
                    break;
            }
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                alert("操作用户的id" + data.userId)
                window.location.href = "${pageContext.request.contextPath}/group/userSanitation.do?userId=" + data.userId;
                layer.msg('查看操作');
            } else if (layEvent === 'detai2') {
                alert("操作用户的id" + data.userId)
                window.location.href = "${pageContext.request.contextPath}/group/userAttendance.do?userId=" + data.userId;
                layer.msg('查看操作');
            }
        });
        //分页
