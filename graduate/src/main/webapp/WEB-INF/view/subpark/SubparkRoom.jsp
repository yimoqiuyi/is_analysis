    ...
    <script>
        ...
        layui.use(['laypage', 'layer', 'table', 'element', 'slider'], function () {
            ...
            table.on('toolbar(test)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        window.location.href = "${pageContext.request.contextPath}/subpark/AddRoom.do"
                        layer.msg('添加');
                        break;
                    case 'update':
                        if (data.length === 0) {
                            ...
                            window.location.href = "${pageContext.request.contextPath}/subpark/selectOneRoom.do?roomId=${checkStatus.data[0].roomId}"
                    ...
                    case 'delete':
                        if (data.length === 0) {
                            ...
                            window.location.href = "${pageContext.request.contextPath}/subpark/deleteRooms.do?roomId=${checkStatus.data[0].roomId}"
                            layer.msg('删除');
                        }
                        break;
                }
            ...
            table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') {
                    window.location.href = "${pageContext.request.contextPath}/subpark/selectOneRoom.do?roomId=" + data.roomId
                    layer.msg('查看操作');
                    ...
                    window.location.href = "${pageContext.request.contextPath}/subpark/updateRoom.do?roomId=" + data.roomId
                    layer.msg('编辑操作');
                }
            });
            //分页
