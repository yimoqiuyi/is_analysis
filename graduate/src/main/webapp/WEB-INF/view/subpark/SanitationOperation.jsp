            ...
            <div style="padding: 15px;">
                <div style="padding: 15px;">
                    <table class="layui-table" lay-even lay-skin="nob">
                        ...
                        <tbody>
                        <c:forEach items="${sanitationList}" var="item">
                            <tr>
                                <td>${item.sId}</td>
                                <td>${item.sanitationCategory}</td>
                                <td>${item.sanitationScore}</td>
                                <td>
                                    <a href="${pageContext.request.contextPath}/subpark/addSanitation.do"
                                       class="layui-btn layui-btn-sm layui-btn-normal">增加</a>
                                    <a href="${pageContext.request.contextPath}/subpark/updateSanitation.do?sId=${item.sId}"
                                       class="layui-btn layui-btn-sm layui-btn-warm">修改</a>
                                    <a href="${pageContext.request.contextPath}/subpark/deleteSanitation.do?sId=${item.sId}"
