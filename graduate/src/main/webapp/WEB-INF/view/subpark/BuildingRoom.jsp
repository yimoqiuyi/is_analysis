        ...
        <c:if test="${not empty sessionScope.user}">
         ...
         <div class="layui-body">
            ...
            <script type="text/html" id="barDemo">
                ...
                <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detai2">修改</a>
                 <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">房间删除</a>
            </script>
    ...
    <script>
        ...
        layui.use(['laypage', 'layer', 'table', 'element', 'slider'], function () {
            ...
            table.render({
                elem: '#demo'
                , height: 420
                , url: 'selectBuildingRoom.do' //数据接口
                , title: '用户表'
                , page: true //开启分页
                ...
                , cols: [[ //表头
                    ...
                    , {field: 'roomPosition', title: '具体位置描述', width: 180}
                    , {field: 'roomPicture', title: '房间平面图', width: 180}
                ...
                switch (obj.event) {
                    ...
                    case 'update':
                        if (data.length === 0) {
                            ...
                            window.location.href = "${pageContext.request.contextPath}/subpark/updateRoomParkassets.do?roomId="+id;
                    ...
                    case 'delete':
                        if (data.length === 0) {
                            ...
                            var id = checkStatus.data[0].roomId;
                            window.location.href = "${pageContext.request.contextPath}/subpark/deleteRooms.do?roomId="+id;
                            layer.msg('删除');
                        }
                        break;
                }
            ...
            table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') {
                    ...
                    layer.confirm('真的删除行么', function (index) {
                        window.location.href = "${pageContext.request.contextPath}/subpark/deleteRooms.do?roomId=" + data.roomId
                        obj.del(); //删除对应行（tr）的DOM结构
                        layer.close(index);
                        //向服务端发送删除指令
                    });
                }else if (layEvent === 'detai2') {
                    window.location.href = "${pageContext.request.contextPath}/subpark/updateRoomParkassets.do?roomId=" + data.roomId
                    layer.msg('编辑操作');
                }
            });
            //分页
