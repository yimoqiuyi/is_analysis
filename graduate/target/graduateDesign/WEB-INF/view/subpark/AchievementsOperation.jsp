            ...
            <div style="padding: 15px;">
                <div style="padding: 15px;">
                    <table class="layui-table" lay-even lay-skin="nob">
                        ...
                        <tbody>
                        <c:forEach items="${achievementsList}" var="item">
                            <tr>
                                <td>${item.achievementsId}</td>
                                <td>${item.achievementsName}</td>
                                <td>${item.achievementsIntroduction}</td>
                                <td>${item.achievementsCategory}</td>
                                <td>${item.achievementsScore}</td>
                                <td>
                                    <a href="${pageContext.request.contextPath}/subpark/addAchievements.do"
                                       class="layui-btn layui-btn-sm layui-btn-normal">增加</a>
                                    <a href="${pageContext.request.contextPath}/subpark/updateAchievements.do?achievementsId=${item.achievementsId}"
                                       class="layui-btn layui-btn-sm layui-btn-warm">修改</a>
                                    <a href="${pageContext.request.contextPath}/subpark/deleteAchievements.do?achievementsId=${item.achievementsId}"
