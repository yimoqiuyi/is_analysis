...
<script>
    ...
    layui.use(['laypage', 'layer', 'table', 'element', 'slider'], function () {
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                case 'add':
                    ...
                    window.location.href = "${pageContext.request.contextPath}/role/insert.do?userId=" + id;
                    layer.msg('添加');
                    break;
                case 'update':
                    if (data.length === 0) {
                        ...
                        var id = checkStatus.data[0].userId;
                        window.location.href = "${pageContext.request.contextPath}/role/person.do?userId=" + id;
                ...
                case 'delete':
                    if (data.length === 0) {
                        ...
                        var id = checkStatus.data[0].userId;
                        window.location.href = "${pageContext.request.contextPath}/role/deleteuser.do?userId=" + id;
                        layer.msg('删除');
                    }
                    break;
            }
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                window.location.href = "${pageContext.request.contextPath}/role/person.do?userId="+data.userId
                layer.msg('查看操作');
            } else if (layEvent === 'del') {
                layer.confirm('真的删除行么', function (index) {
                    window.location.href = "${pageContext.request.contextPath}/role/deleteuser.do?userId="+data.userId
